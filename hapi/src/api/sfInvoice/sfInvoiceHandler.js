...
const model = require('./sfInvoiceModel');
const schema = require('./sfInvoiceSchema');
const ReplyHelper = require('src/base/reply-helper');

function sfInvoiceHandler() {
    ...
    self.insert = (request, reply) => {
        ...
        p.params.dcDate = p.params.dcDate ? p.params.dcDate.replace(/T00:00:00/, '').replace(/\..+/, '') : null;
        const params = {
            params: p.params,
...
const handler = new sfInvoiceHandler();
