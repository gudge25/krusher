...
SqlString.toExec = function(storedProc, val) {
  const params = SqlString.escape(val);
  const sql = `call ${storedProc}(${params});`;
  console.log(sql);
  return sql;
};
...
SqlString.objectToValues = function(object, timeZone) {
    ...
    if (object.Data) {
        for (key in object.Data) {
            ...
            if ((typeof value === 'function' || typeof value === `object` || value === undefined) && value !== null) {
                continue;
            }

            values.push(SqlString.escape(value, true, timeZone));
        }
    } else {
        for (key in object) {
            value = object[key];
