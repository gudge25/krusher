...
BEGIN
  ...
  ELSE
    ...
    SET $sql = CONCAT('SELECT
          ...
          , c.SIP                                     SIP
          , IF(c.ccStatus = 7001, CONCAT("/monitor/", c.LinkFile, ".wav"), "") LinkFile
    ...
    END IF;
    --
    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.Aid = ', $Aid, ' AND c.CallType != 101320 AND LENGTH(c.ccName)>5 AND c.SIP IS NOT NULL AND c.isActive = TRUE');
    --
    ...
    END IF;
    --
    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.Aid = ', $Aid, ' AND c.CallType != 101320 AND LENGTH(c.ccName)>5 AND c.SIP IS NULL AND c.isActive = TRUE');
    --
    IF $isMissed = 1 or $isUnique = 1 then
      SET $sqlGroup = CONCAT(CHAR(10), 'GROUP BY c.ccName');
    END IF;
    ...
    SET $sqlRes = CONCAT($sqlHeader, ' ((', @s, ') UNION ALL (', @s2, '))a
                          ...
                          /*'ORDER BY a.Created DESC LIMIT 10000');*/
    SET @s3 = $sqlRes;
    /*select @s3;*/
