DROP PROCEDURE IF EXISTS em_GetStatEmploy;
DELIMITER $$
CREATE PROCEDURE em_GetStatEmploy(
    $token          VARCHAR(100)
    , $DateFrom     DATETIME
    , $DateTo       DATETIME
    , $emIDs        TINYTEXT
...
BEGIN
  ...
  IF ($Aid = -999) THEN
    call RAISE(77068, 'em_GetStatEmploy');
  ELSE
    ...
    SET $sql2 = CONCAT('INSERT INTO ', IF($step = 1, '`dashboard_datetime`', IF($step = 2, '`dashboard_date`', '`dashboard_step`')), '
      ...
      WHERE d.Created >= "', $dateFrom, '"
        ...
        ', IF($IsOut IS NOT NULL, CONCAT('AND cc.IsOut = ', IF($IsOut = TRUE, 1, 0)), ''), '
        AND ee.Aid = ', $Aid, ' AND LENGTH(cc.ccName)>5
        ', IF($dctID = 1, CONCAT('AND cc.CallType != 101320'), ''), '
      GROUP BY
        ee.emID
        , ee.emName
        , t.dctName
