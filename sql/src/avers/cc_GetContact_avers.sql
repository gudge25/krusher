...
BEGIN
  ...
  ELSE
    ...
    SET $sql = CONCAT('SELECT
                            ...
                            , c.SIP                     SIP
                            , IF(c.CallType IS NOT NULL AND c.CallType = 101312, (SELECT text_sms FROM cc_Sms WHERE dcID = c.dcID), c.LinkFile)                LinkFile
                            , c.ffID                    ffID
    ...
    END IF;
    --
    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.Aid = ', $Aid, ' AND c.CallType != 101320 AND LENGTH(c.ccName)>5 AND c.SIP IS NOT NULL AND (c.destination IS NULL OR c.destination != 101412) AND c.isActive = TRUE');
    --
    ...
    END IF;
    --
    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.Aid = ', $Aid, ' AND c.CallType != 101320 AND LENGTH(c.ccName)>5 AND c.SIP IS NULL AND (c.destination IS NULL OR c.destination != 101412) AND c.isActive = TRUE');
    --
    IF $isMissed = 1 or $isUnique = 1 then
      SET $sqlGroup = CONCAT(CHAR(10), 'GROUP BY c.ccName');
    END IF;
