DROP PROCEDURE IF EXISTS cc_GetContact;
DELIMITER $$
CREATE PROCEDURE cc_GetContact(
    $token              VARCHAR(100)
    , $DateFrom         DATETIME
    , $DateTo           DATETIME
    , $dcIDs            TINYTEXT
...
BEGIN
    ...
    IF ($Aid = -999) THEN
        call RAISE(77068, 'cc_GetContact');
    ELSE
        ...
        SET $sql = CONCAT('SELECT
                            ...
                            , c.SIP                     SIP
                            , IF(c.CallType IS NOT NULL AND c.CallType = 101312, (SELECT text_sms FROM cc_Sms WHERE dcID = c.dcID LIMIT 1), c.LinkFile)                LinkFile
                            , c.ffID                    ffID
        ...
        IF (($ContactStatuses is NOT NULL) AND (length(TRIM($ContactStatuses))>0)) THEN
            ...
            IF ((LOCATE('true', $ContactStatuses)>0) OR (LOCATE('false', $ContactStatuses)>0)) THEN
                IF (LOCATE(',', $ContactStatuses)>0) THEN
                    SET $sqlMaker = REPLACE($ContactStatuses, ',true', ') OR c.ContactStatus = 0');
                    SET $sqlMaker = REPLACE($sqlMaker, ',false', ') OR c.ContactStatus != 0');
                    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.ContactStatus IN (', $sqlMaker);
                ELSE
                    IF $ContactStatuses = 'true' THEN
                        SET $sqlMaker = 'c.ContactStatus = 0';
                    END IF;
                ...
                END IF;
            ELSE
                SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.ContactStatus IN (', $ContactStatuses, ')');
            END IF;
            SET $sqlWhereCode = ' AND ';
        END IF;
        IF $isMissed = 1 THEN
        ...
        IF $destdata2 is NOT NULL THEN
            ...
            SET $sqlWhereCode = ' AND ';
        END IF;
        IF (($target is NOT NULL) AND (length(TRIM($target))>0)) THEN
            SET $sqlMaker = '';
            IF ((LOCATE('true', $target)>0) OR (LOCATE('false', $target)>0)) THEN
                IF (LOCATE(',', $target)>0) THEN
                    SET $sqlMaker = REPLACE($target, ',true', ') OR target IS NOT NULL');
                    SET $sqlMaker = REPLACE($sqlMaker, ',false', ') OR target IS NULL');
                    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.target IN ("', $sqlMaker);
                ELSE
                    IF $emIDs = 'true' THEN
                        SET $sqlMaker = 'target IS NOT NULL';
                    END IF;
                    IF $emIDs = 'false' THEN
                        SET $sqlMaker = 'target IS NULL';
                    END IF;
                    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, $sqlMaker);
                END IF;
            ELSE
                SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.target IN ("', REPLACE($target, ',', '","'), '")');
            END IF;
            SET $sqlWhereCode = ' AND ';
        END IF;
        ...
        IF (($coIDs is NOT NULL) AND (length(TRIM($coIDs))>0)) THEN
            ...
            IF ((LOCATE('true', $coIDs)>0) OR (LOCATE('false', $coIDs)>0)) THEN
                IF (LOCATE(',', $coIDs)>0) THEN
                    SET $sqlMaker = REPLACE($coIDs, ',true', ') OR c.coID = 0');
                    SET $sqlMaker = REPLACE($sqlMaker, ',false', ') OR c.coID != 0');
                    SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'c.coID IN (', $sqlMaker);
                ELSE
                    IF $coIDs = 'true' THEN
    ...
    --    select $sqlRes2;
        IF $isMissed = 1 or $isUnique = 1 then
            SET @s = CONCAT("SELECT count(IsOut) Qty
                              ...
                              , SUBSTR(sec_to_time(Avg(avgHoldMin)), 1,8) avgHoldMin
                      FROM (", $sqlRes2, ") dd");
        ELSE
            SET @s = CONCAT("SELECT count(IsOut) Qty
                              , sec_to_time(Sum(
