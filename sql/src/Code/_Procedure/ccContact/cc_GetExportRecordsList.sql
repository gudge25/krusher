DELIMITER $$
DROP PROCEDURE IF EXISTS cc_GetExportRecordsList;
CREATE PROCEDURE cc_GetExportRecordsList(
    $token                VARCHAR(100)
    , $DateFrom           DATETIME
    , $DateTo             DATETIME
    , $statusReady        VARCHAR(50)
    , $convertFormat       INT
    , $isActive           BIT
    , $sorting            VARCHAR(5)
    , $field              VARCHAR(50)
...
BEGIN
    ...
    IF ($Aid = -999) THEN
        call RAISE(77068, 'cc_GetExportRecordsList');
    ELSE
        ...
        IF $DateTo is NOT NULL THEN
            ...
            SET $sqlWhereCode = ' AND ';
        END IF;
        IF $statusReady is NOT NULL THEN
            SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode,  'statusReady = ', QUOTE($statusReady));
            SET $sqlWhereCode = ' AND ';
        END IF;
        IF $convertFormat is NOT NULL THEN
            SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode,  'convertFormat = ', QUOTE($convertFormat));
            SET $sqlWhereCode = ' AND ';
        END IF;
