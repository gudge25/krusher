DROP PROCEDURE IF EXISTS cc_GetExportDailyHour;
DELIMITER $$
CREATE PROCEDURE cc_GetExportDailyHour(
    $token          VARCHAR(100)
      , $DateFrom         DATETIME
      , $DateTo           DATETIME
      , $emIDs            TINYTEXT
...
BEGIN
    ...
    IF ($Aid = -999) THEN
        call RAISE(77068, 'cc_GetExportDailyHour');
    ELSE
...
, IF(count(dcID) = 0 OR SUM(CASE WHEN ((duration > 5) AND ccStatus NOT IN (7001)) THEN 1 ELSE 0 END) = 0, 0, round((((SUM(CASE WHEN ((duration < 5 OR serviceLevel IS NULL) AND ccStatus NOT IN (7001)) THEN 1 ELSE 0 END)*100)/(count(dcID)))), 2))		LCR
                             ...
                             /*, IF(SUM(CASE WHEN ccStatus IN (7001) THEN 1 ELSE 0 END) = 0, 0, SUM(CASE WHEN (ccStatus IN (7001)) THEN duration ELSE 0 END))		ATT2*/
                             , Aid
                             , Created
        ...
        /*IF (($channels is NOT NULL) AND (length(TRIM($channels))>0)) THEN
            ...
            ELSE
                SET $sqlMaker = REPLACE($channels, '"', '');
                SET $sql = CONCAT($sql, CHAR(10), $sqlWhereCode, 'channel  = "', $sqlMaker, '"');
            END IF;
            SET $sqlWhereCode = ' AND ';
        END IF;*/
