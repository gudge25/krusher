DELIMITER $$
DROP PROCEDURE IF EXISTS ast_GetQueue;
CREATE PROCEDURE ast_GetQueue(
  $token                          VARCHAR(100)
  , $queID                        INT(11)
  , $name                         VARCHAR(128)
...
BEGIN
  ...
  ELSE
    ...
    IF $context is NOT NULL THEN
      ...
      SET $sqlWhereCode = ' AND ';
    END IF;
    IF $max_wait_time is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'max_wait_time = ', $max_wait_time);
      SET $sqlWhereCode = ' AND ';
    END IF;
    IF $fail_destination is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'fail_destination = ', $fail_destination);
      SET $sqlWhereCode = ' AND ';
    END IF;
    IF $fail_destdata is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'fail_destdata = ', $fail_destdata);
      SET $sqlWhereCode = ' AND ';
    END IF;
    IF $fail_destdata2 is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'fail_destdata2 = ', QUOTE($fail_destdata2));
      SET $sqlWhereCode = ' AND ';
    END IF;
