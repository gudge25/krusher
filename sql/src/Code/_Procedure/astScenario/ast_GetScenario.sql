DELIMITER $$
DROP PROCEDURE IF EXISTS ast_GetScenario;
CREATE PROCEDURE ast_GetScenario(
    $token                          VARCHAR(100)
    , $id_scenario                  INT(11)
...
BEGIN
  ...
  IF ($Aid = -999) THEN
    call RAISE(77068, 'ast_GetScenario');
  ELSEIF ($Aid = 0) THEN
    ...
    IF $DaysCall is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode,  's.DaysCall IN (', QUOTE($DaysCall),')');
      SET $sqlWhereCode = ' AND ';
    END IF;
    IF $limitStatuses is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode,  's.limitStatuses IN (', QUOTE($limitStatuses),')');
      SET $sqlWhereCode = ' AND ';
    END IF;
    IF $limitChecker is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode,  's.limitChecker = ', $limitChecker);
      SET $sqlWhereCode = ' AND ';
    END IF;
