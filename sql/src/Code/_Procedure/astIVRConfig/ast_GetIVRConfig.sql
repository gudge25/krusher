DELIMITER $$
DROP PROCEDURE IF EXISTS ast_GetIVRConfig;
CREATE PROCEDURE ast_GetIVRConfig(
  $token                        VARCHAR(100)
...
BEGIN
  ...
  IF ($Aid = -999) THEN
    call RAISE(77068, 'ast_GetIVRConfig');
  ELSE
    ...
    END IF;
    --
    SET $sqlCount = 'SELECT count(*) Qty FROM ast_ivr_config';
    --
    SET $sql = '
            SELECT
              HIID
    ...
    IF $record_id is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'record_id = ', QUOTE($record_id));
      SET $sqlWhereCode = ' AND ';
    END IF;
    ...
    IF $retry_record_id is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'retry_record_id = ', QUOTE($retry_record_id));
      SET $sqlWhereCode = ' AND ';
    END IF;
    ...
    IF $invalid_record_id is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'invalid_record_id = ', QUOTE($invalid_record_id));
      SET $sqlWhereCode = ' AND ';
    END IF;
    ...
    IF $timeout_retry_record_id is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'timeout_retry_record_id = ', QUOTE($timeout_retry_record_id));
      SET $sqlWhereCode = ' AND ';
    END IF;
    ...
    IF $timeout_record_id is NOT NULL THEN
      SET $sqlWhere = CONCAT($sqlWhere, CHAR(10), $sqlWhereCode, 'timeout_record_id = ', QUOTE($timeout_record_id));
      SET $sqlWhereCode = ' AND ';
    END IF;
