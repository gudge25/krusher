<?php
    function fop2_sync($ARG1, $ARG2, $link)
    {
        ...
        if (file_exists($file))
        {
            ...
            {
                unset($domain);
                $domain = explode('.', $row['url']);
                unset($domain[count($domain)-1]);
                unset($domain[count($domain)-1]);
                $list[$row['id_client']] = implode('', $domain);
            }
            ...
            foreach ($list as $key => $item)
            {
                ...
                $query = "SELECT trID, Aid, trName, uniqName FROM ast_trunk WHERE Aid=".$key." AND isActive = TRUE AND Aid IN (SELECT id_client FROM emClient WHERE isActive = TRUE) ORDER BY trID;";
                //echo $query."\r\n";
                $result2 = mysqli_query($link, $query) or die("Query fail3: " . mysqli_error($link));
                while ($row = mysqli_fetch_array($result2))
                    $list4[] = 'SIP/'.$row['uniqName'];
                /*$query = "SELECT * FROM ast_route_outgoing r
                        INNER JOIN ast_route_outgoing_items i ON i.roID=r.roID
                        INNER JOIN ast_conference c ON c.cfID=r.destdata
                        WHERE destination = 101412 AND c.isActive = TRUE AND r.isActive = TRUE AND c.Aid = ".$key." AND c.Aid IN (SELECT id_client FROM emClient WHERE isActive = TRUE);";*/
